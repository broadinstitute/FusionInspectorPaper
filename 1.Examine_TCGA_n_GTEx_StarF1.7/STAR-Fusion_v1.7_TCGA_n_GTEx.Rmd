...
data$fusion_name = str_replace(data$fusion_name, "--", "::")
...
# require reference splicing for non-cosmic
data = data %>% filter(SpliceType == "ONLY_REF_SPLICE" | cosmic)
...
get_fusion_counts_vs_min_FFPM_via_tissue_type = function (data, min_ffpm_vals = seq(0, 0.3, 0.025)) {
    ...
    samples_n_tissues = data %>%   
      ...
      select(data_class, dataclassTN, tissue_type, sample) %>% unique()
    ...
    for (min_ffpm in min_ffpm_vals) {
        ...
        counts_per_filt_sample = data_filt %>% select(data_class, dataclassTN, tissue_type, sample, fusion_name) %>% 
            unique() %>% group_by(data_class, dataclassTN, tissue_type, sample) %>% tally(name='fusion_count') %>% ungroup()
        
        counts_per_filt_sample = left_join(samples_n_tissues, counts_per_filt_sample, 
                                           by=c('data_class', 'dataclassTN', 'tissue_type', 'sample')) %>%
...
median_tissue_fusion_counts = tissue_fusion_counts_vs_min_ffpm  %>% 
    ...
    summarise_at(vars(fusion_count), list(median_fusion_count = median)) %>% ungroup()
...
tcga_sample_info = data %>% filter(data_class == "TCGA") %>% select(tissue_type, sample) %>% unique()
tcga_sample_info$patient = str_replace(tcga_sample_info$sample, "-(NT|TP)$", "")
patients_with_both_tumor_normal_samples = tcga_sample_info %>% group_by(patient) %>% tally() %>% filter(n>1) %>% pull(patient)

tissue_fusion_counts_vs_min_ffpm$patient = str_replace(tissue_fusion_counts_vs_min_ffpm$sample, "-(NT|TP)$", "")
...
data2 = data %>% mutate(dataclass2 = dataclassTN) %>% 
    mutate(dataclass2 = ifelse(cosmic, "Cosmic", dataclass2))
...
p = df_FFPM %>% mutate(FFPM_max1 = pmin(1, FFPM)) %>% 
  ggplot(aes(x=FFPM_max1, color=dataclass)) + 
  geom_density(alpha=0.2) + xlim(0, 1) + ylab("Density") + xlab("FFPM") +
  theme_bw() +
...
ggsave("fusion_expr_density.svg", p, width=7.65, height=5.1)
...
cosmic_fusion_occurrence = data2 %>% filter(data_class == "GTEx" | tumor_or_normal == "tumor") %>% 
    ...
    select(fusion_name, sample, data_class, tissue_type) %>% 
    unique() %>% 
...
sample_tissue_counts = data2 %>% filter(data_class == "GTEx" | tumor_or_normal == "tumor") %>% 
    select(sample, data_class, tissue_type) %>% 
    mutate(sample_tissue = paste(data_class, tissue_type)) %>% 
    select(sample, sample_tissue) %>% 
    unique() %>% 
    group_by(sample_tissue) %>% 
    tally(name='sample_tissue_count') %>% ungroup()
...
# when we find COSMIC fusions in GTEx normal samples, they are found at low frequencies (< 1% prevalence in a tissue type).
...
```{r}
 ...
 data2 %>% filter(dataclass2 == "Cosmic") %>% select (fusion_name) %>% unique() %>% nrow()
