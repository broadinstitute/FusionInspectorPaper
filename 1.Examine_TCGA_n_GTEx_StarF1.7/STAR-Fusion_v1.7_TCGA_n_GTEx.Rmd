...
min_ffpms = median_tissue_fusion_counts %>% select(min_ffpm) %>% unique() %>% filter(min_ffpm >= 0.1) %>% arrange(min_ffpm) %>% pull(min_ffpm)
...
for (min_ffpm_use in min_ffpms) {
  ...
  tumor_normal_paired_fusion_counts = tissue_fusion_counts_vs_min_ffpm %>% 
   ...
   for (tissue_type_selected in tissue_types) {
     ...
     t = t.test(normal_counts, tumor_counts, paired = T)
