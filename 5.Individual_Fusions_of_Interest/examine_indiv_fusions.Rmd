...
FI_samples = read.table("../data/FusionInspector.v2.4.0.C4_ =targeted_fusions.samples.list", header=F, stringsAsFactors = F)[,1]
...
person_counts_per_tissue_type = orig_STARF_fusion_preds %>% 
    select(tissue_type, data_class, patient_name) %>% 
    unique() %>% 
    group_by(tissue_type, data_class) %>% 
    tally(name='person_counts')

```


# Examine some specific fusions.
...
examine_sample_fractions_with_fusion = function(input_fusion_name) {
    ...
    p = count_table %>% ggplot(aes(x=person_counts, y=frac)) + 
        geom_point(aes(color=data_class)) +       
...
# TMPRSS2--ERG in orig starF findings:

orig_STARF_fusion_preds %>% filter(fusion_name == "TMPRSS2--ERG") %>% filter(is_primary) %>% group_by(data_class, tumor_or_normal, tissue_type) %>% tally()
```
...
examine_sample_fractions_with_fusion("TMPRSS2--ERG")
```
...
examine_sample_fractions_with_fusion("FGFR3--TACC3")
...
fusion_data %>% filter(fusion_name == "PVT1--MYC") %>% 
    filter(is_primary) %>% 
    filter(FFPM >= 0.01) %>% 
    group_by(tumor_or_normal) %>% tally()
...
# but filter on expression first:

pvt1_myc_normal_ffpms = pvt1_myc_normal_ffpms [pvt1_myc_normal_ffpms >= 0.01]
pvt1_myc_tumor_ffpms = pvt1_myc_tumor_ffpms[pvt1_myc_tumor_ffpms >= 0.01]
wilcox.test(pvt1_myc_tumor_ffpms, pvt1_myc_normal_ffpms, alternative = 'greater')
...
```
```{r}
# expr view from the orig starF data:
...
```{r}
examine_sample_fractions_with_fusion("PVT1--MYC")
```
Compare CESC expression to all other samples

```{r}

# using the orig starF data

pvt1_myc_CESC_ffpms = orig_STARF_fusion_preds %>% filter(fusion_name == "PVT1--MYC") %>% filter(is_primary) %>% filter(tissue_type == "CESC") %>% pull(FFPM)

pvt1_myc_OTHER_ffpms = orig_STARF_fusion_preds %>% filter(fusion_name == "PVT1--MYC") %>% filter(is_primary) %>% filter(tissue_type != "CESC") %>% pull(FFPM)
