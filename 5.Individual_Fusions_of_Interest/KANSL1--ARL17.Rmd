...
samples_with_KANSL1_ARL17 = orig_STARF_fusion_preds %>% filter(grepl("KANSL1::ARL17", fusion_name)) %>% select(sample) %>% unique() %>% mutate(ARL17=TRUE)

message("Num samples with KANSL1_ARL17: ", nrow(samples_with_KANSL1_ARL17))

samples_with_KANSL1_LRRC37 = orig_STARF_fusion_preds %>% filter(grepl("KANSL1::LRRC37", fusion_name)) %>% select(sample) %>% unique() %>% mutate(LRRC37 = TRUE)
