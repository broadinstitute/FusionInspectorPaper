...
# Are the TCGA TMPRSS2--ERG containing normal samples also having the fusion in the matched tumors?

participants_with_TMPRSS2_ERG_fusion = fusion_data %>% filter(fusion_name == "TMPRSS2::ERG") %>% filter(is_primary) %>% filter(tumor_or_normal == "normal") %>% select(sample) %>% unique() %>% mutate(participant = str_replace(sample, "-[^\\-]+$", ""))  %>% pull(participant)


fusion_data %>% filter(fusion_name == "TMPRSS2::ERG") %>%  mutate(participant = str_replace(sample, "-[^\\-]+$", "")) %>% filter(participant %in% participants_with_TMPRSS2_ERG_fusion) %>% filter(is_primary) %>% arrange(sample)

# all 5 prostate noraml samples with TMPRSS2--ERG are also found in the corresponding tumors.
...
fusion_data %>% filter(grepl("SSX", fusion_name)) %>% filter(grepl("SS18", fusion_name)) %>% 
    filter(is_primary) %>% select(fusion_name, pred_cluster)


# COSMIC fusions SS18::SSX1 and SS18::SSX2, known drivers of synovial sarcoma (Clark et al., 1994, Gazendam et al., 2021), are in other clusters (C38 & C39)
