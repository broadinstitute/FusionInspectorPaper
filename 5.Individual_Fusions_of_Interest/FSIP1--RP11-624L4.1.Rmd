...
orig_starF_preds %>% filter(fusion_name == "FSIP1--RP11-624L4.1") %>% filter(is_primary) %>%
    ggplot(aes(x=reorder(sample_name, FFPM), y=FFPM, fill=tumor_or_normal)) + geom_col()
...
expr_matrix_tidy %>% ggplot(aes(x=FSIP1, y=RP11.624L4.1)) + geom_point(alpha=0.5, aes(color=tumor_or_normal)) +
