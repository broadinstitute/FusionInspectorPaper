...
orig_starF_preds %>% filter(tumor_or_normal == "tumor" & tissue_type == "BRCA") %>% select(data_class, sample) %>% unique() %>% count()

# 240/1086 = 22% of BRCA tumor samples
...
fusion_data = left_join(fusion_data, orig_starF_preds %>% filter(fusion_name == "FSIP1::RP11-624L4.1") %>% select(sample, tumor_or_normal) %>% unique(), by='sample')
...
p = orig_starF_preds %>% filter(fusion_name == "FSIP1::RP11-624L4.1") %>% filter(is_primary) %>%
    ggplot(aes(x=reorder(sample, FFPM), y=FFPM, fill=tumor_or_normal)) + geom_col() + xlab(paste0(num_brca_samples, " BRCA samples ranked by FSIP1::RP11-624L4.1 FFPM")) + theme(axis.text.x=element_blank())
...
samples_with_fusion = orig_starF_preds %>% filter(fusion_name == "FSIP1::RP11-624L4.1") %>% select(sample) %>% unique() %>% pull(sample)
