...
# Evaluating fusion cluster prediction accuracy
...
write.table(data.scaled, file="fusion_atts_scaled_values.tsv", row.names=F, sep="\t", quote=F)
...
prediction_accuracies_df_stats %>%  ggplot(aes(x=Var1, y=mean_pct)) +  
  ...
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5, size=rel(1) )) +
  xlab("Fusion Cluster") +
  ylab("Cluster Prediction Accuracy")
...
save( all_median_vals_df, colclust, plot_heatmap_n_fusion_counts, cosmic_counts_per_cluster, file="share_data.Rdata")
