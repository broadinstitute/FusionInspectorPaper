...
fusion_data %>% nrow()
# 53240 fusion variants

fusion_data %>% filter(is_primary) %>% nrow()
# 38591 primary fusion occurrences

fusion_data %>% filter(! is_primary) %>% nrow()
# 14649 alt fusion variants

```


```{r}
# Can experiment with the code below to control for fusions having high prevalences among the samples. C4 still looks special here.
# fusion_data = fusion_data %>% filter(is_primary) %>% group_by(fusion_name) %>% filter(row_number() <= 3) %>% ungroup()  ## TESTING
...
pheatmap(all_median_vals_df[rownames(all_median_vals_df) %in% high_counter_evidence_clusters,],
        cluster_cols=colclust,
        main="high counter-evidence clusters" 
             )
...
fusion_data = fusion_data %>% mutate(fusion_cluster_att = ifelse(leiden %in% high_counter_evidence_clusters, "high-counter-evidence", fusion_cluster_att))
...
# fusion_cluster_att  n    pct
# cosmic-like	1511	0.03915421
# expr_microH_RT_artifact?	756	0.01959006
# high_FAR_microH_bioinf_artifact?	444	0.01150527
# high-counter-evidence	373	0.009665466
# NA	35880	0.92975046
...
artifact_fusion_data = fusion_data %>% filter(is_primary) %>% group_by(fusion_cluster_att) %>% tally() %>% mutate(pct=prop.table(n)) %>%
    filter(fusion_cluster_att %in% c('expr_microH_RT_artifact?', 'high_FAR_microH_bioinf_artifact?')) 

artifact_fusion_data

# Of the 3% of all fusion occurrences encompassed by these clusters: 2/3 (2% of all fusion occurrences) had moderately-to-highly expressed partner genes, suggesting origination from RT mis-priming, and  the other 1/3 with little evidence for partner gene expression, suggesting read misalignments artifacts.
...
# expr_microH_RT_artifact?	756	0.01959006	0.63	
# high_FAR_microH_bioinf_artifact?	444	0.01150527	0.37
```
...
cosmic_cluster_info_stats$pval_BH = p.adjust(cosmic_cluster_info_stats$pval, method="BH")

cosmic_cluster_info_stats %>% filter(yes_cosmic >= 2)

# cluster 4 has 4% of all fusion occurrences and 57% of all cosmic fusion occurrences, enrichment p<1e-90 (uncorrected)
...
# sum(pct_all_cosmic_fusions) sum(pct_tot_fusions)
# 0.372093	0.3424114
