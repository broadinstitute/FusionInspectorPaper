...
p = fusion_data %>% ggplot(aes(x = umap1, y = umap2)) + 
...
# start with all fusions
fusion_data %>% group_by(fusion_cluster_att) %>% tally() %>% mutate(pct=prop.table(n))

```

```{r}
# restrict to primary fusions
...
cosmic_fusions_in_C4 = fusion_data %>% filter(leiden == 4) %>% filter(cosmic) %>% select(fusion_name) %>% unique() %>% pull(fusion_name)

fusion_data %>% filter(fusion_name %in% cosmic_fusions_in_C4) %>% filter(leiden==4) %>% gather(key=FAR_type, value=FAR_val, FAR_left, FAR_right) %>%
