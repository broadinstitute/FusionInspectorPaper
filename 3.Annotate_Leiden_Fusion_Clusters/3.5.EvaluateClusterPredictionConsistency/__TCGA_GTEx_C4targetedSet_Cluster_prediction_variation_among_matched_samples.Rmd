...
data = data %>% mutate(fusion_cluster_att = ifelse(pred_cluster == 4, "cosmic-peak-enriched", fusion_cluster_att)) # highlight C4 
...
gtex_data_matched_fusions %>% select(participant, sample_name, fusion_variant, pred_cluster, fusion_cluster_att)
...
combined_results = full_join(data_matched_fusions %>% select(participant, sample_name, fusion_variant, pred_cluster, fusion_cluster_att),
                             data_matched_fusions %>% select(participant, sample_name, fusion_variant, pred_cluster, fusion_cluster_att),
                             by = c('participant', 'fusion_variant') ) %>%
...
# count tissue representation

artif_fusion_variants %>% ggplot(aes(x=tissue_type, y=n)) + geom_col()
