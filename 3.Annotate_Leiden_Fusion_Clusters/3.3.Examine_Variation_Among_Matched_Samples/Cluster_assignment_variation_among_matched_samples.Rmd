...
tcga_samples_with_matched_fusions = tcga_data %>% filter(is_primary) %>% group_by(participant, fusion_name) %>% tally() %>% filter(n>1)

tcga_data_matched_fusions = left_join(tcga_samples_with_matched_fusions %>% select(participant, fusion_name),
                                      tcga_data %>% filter(is_primary),
...
tcga_data_matched_fusions %>% select(participant, fusion_name, type, leiden, fusion_cluster_att)


```


Not very many examples from our sample selection from TCGA.  Moving on to GTEx where we have many samples (different tissues) from participants.
...
gtex_samples_with_matched_fusions = gtex_data %>% filter(is_primary) %>% group_by(participant, fusion_name) %>% tally() %>% filter(n>1)

gtex_data_matched_fusions = left_join(gtex_samples_with_matched_fusions %>% select(participant, fusion_name),
                                      gtex_data %>% filter(is_primary),
                                      by=c('participant', 'fusion_name'))
...
gtex_data_matched_fusions %>% select(participant, sample_name, fusion_name, leiden, fusion_cluster_att)

```

```{r}

combined_results = full_join(gtex_data_matched_fusions %>% select(participant, sample_name, fusion_name, leiden, fusion_cluster_att),
                             gtex_data_matched_fusions %>% select(participant, sample_name, fusion_name, leiden, fusion_cluster_att),
                             by = c('participant', 'fusion_name') ) %>%
