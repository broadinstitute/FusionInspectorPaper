...
starF_COSMIC_data = read.table(gzfile("../data/TCGA_n_GTEx.STAR-Fusion.v1.7.fusion_preds.decorated.COSMIC.tsv"), header=T, sep="\t", stringsAsFactors = F)
...
samples_to_process = starF_containing_missed_COSMIC %>% select(sample_name) %>% unique() %>% pull(sample_name)
...
new_data_COSMIC = new_data_COSMIC %>% group_by(sample_name, fusion_name) %>% arrange(desc(FFPM)) %>% filter(row_number() == 1) %>% ungroup()
