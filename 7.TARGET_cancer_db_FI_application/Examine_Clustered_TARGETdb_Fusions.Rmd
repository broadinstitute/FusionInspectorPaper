...
cosmic_entries_plot = top_new_candidates %>% select(fusion_name, sum_sum_frac, cosmic) %>% unique() %>%
  ...
  ggplot(aes(x=reorder(fusion_name, -sum_sum_frac), y=TRUE, fill=cosmic)) + geom_tile() +
...
# examine reciprocal translocations


reciprocal_fusions_data = data %>% filter(is_primary) %>% group_by(sample, sorted_fusion_name) %>% tally() %>% filter(n>1) %>% ungroup()

reciprocal_fusions_data = left_join(reciprocal_fusions_data, data, by=c('sample', 'sorted_fusion_name'))
...
reciprocal_fusion_plot = top_new_candidates %>% select(fusion_name, sum_sum_frac, reciprocal_fusion) %>% unique() %>%
  ggplot(aes(x=reorder(fusion_name, -sum_sum_frac), y=TRUE, fill=reciprocal_fusion)) + geom_tile() +
            theme(axis.title.x=element_blank(), axis.text.x=element_blank(), axis.ticks.x=element_blank()) +
            theme(axis.title.y=element_blank(), axis.text.y = element_blank(), axis.ticks.y=element_blank()) +
...
pg = plot_grid(cosmic_entries_plot,
               reciprocal_fusion_plot,
              candidates_sample_frac_plot,
                 nrow=3,
                 align='v',
                 axis='lr',
                 rel_heights = c(0.15, 0.15,0.70)
                 )
...
chrom_structure_plot = left_join(top_new_candidates, sum_sum_frac) %>% select(fusion_name, structure_type, sum_sum_frac)  %>% unique() %>%
    ggplot(aes(x=reorder(fusion_name, -sum_sum_frac), y=0, fill=structure_type)) + geom_tile() +
       theme(axis.title.x=element_blank(), axis.text.x=element_blank(), axis.ticks.x=element_blank(),
